...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!userId) return NextResponse.json({ message: "Morate biti ulogovani da biste izvrsili ovu akciju" }, { status: 401 });
        const userRole = req.headers.get("x-user-role");
        if (userRole !== Role.ADMIN) return NextResponse.json({ message: "Samo admin moze brisati korisnike" }, { status: 403 });
        const { id } = await params;
        const targetUserId = parseInt(id);
        if (isNaN(targetUserId)) return NextResponse.json({ message: "Nevalidan ID" }, { status: 400 });
        if (targetUserId === parseInt(userId)) return NextResponse.json({ message: "Ne mozete brisati sami sebe" }, { status: 400 });
        const targetUser = await db.user.findUnique({
            where: { id: targetUserId },
            select: { role: true }
        });
        if (!targetUser) return NextResponse.json({ message: "Korisnik nije pronadjen" }, { status: 404 });
        if(targetUser.role === Role.ADMIN) return NextResponse.json({ message: "Ne mozete brisati admina" }, { status: 403 });
        await db.user.delete({
            where: { id: targetUserId }
        });
        return NextResponse.json({ message: "Korisnik uspesno obrisan" });
    } catch (error) {
        return NextResponse.json({ message: error instanceof Error ? error.message : "Greska pri brisanju korisnika" }, { status: 500 });
    }
}